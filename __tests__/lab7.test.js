describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
     ...
     * Remember to remove the .skip from this it once you are finished writing this test.
     */
    // First item is already clicked from step 2, so start from index 1
    await page.$$eval('product-item', (items) => {
      items.slice(1).forEach(item => item.shadowRoot.querySelector('button').click());
    });
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const texts = await page.$$eval('product-item', (items) => {
      return items.map(item => item.shadowRoot.querySelector('button').innerText);
    });
    const allRemove = texts.every(text => text === 'Remove from Cart');
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    await page.$$eval('product-item', (items) => {
      items.forEach(item => item.shadowRoot.querySelector('button').click());
    });
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allAdd = texts.every(text => text === 'Add to Cart');
